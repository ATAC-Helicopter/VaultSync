...
- The prepared stable release target is `1.8.3`; prerelease builds for the active patch train use `1.8.3-Beta.N` until the stable release is cut.
...
2. Build Linux archives:
   ```bash
   bash scripts/build_linux_release.sh 1.8.3 x64 src/VaultSync.UI/bin/Release/net10.0/linux-x64/publish
   bash scripts/build_linux_release.sh 1.8.3 arm64 src/VaultSync.UI/bin/Release/net10.0/linux-arm64/publish
   ```
...
- `previous_version = 1.8.3-Beta.1`
- `target_version = 1.8.3-Beta.2`
...
- Run the release gate before publishing:
  ```powershell
  powershell -ExecutionPolicy Bypass -File scripts/release_readiness_gate.ps1 -TargetVersion 1.8.3 -ReleaseTrack 1.8.x -TargetMilestone 1.8.3
  ```
- Run the release gate again after GitHub Actions uploads assets:
  ```powershell
  powershell -ExecutionPolicy Bypass -File scripts/release_readiness_gate.ps1 -TargetVersion 1.8.3 -ReleaseTrack 1.8.x -TargetMilestone 1.8.3 -Phase PostPublish
  ```
