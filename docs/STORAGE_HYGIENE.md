...
| Per-user application-data `VaultSync` directory | Main database, credentials, installation identity, UI preferences, diagnostics, caches, updater working data, and macOS managed mount points | Durable state is retained. Disposable subdirectories follow the limits below. The `mounts` subtree is never automatically deleted. |
...
| `cache/release-assets` | Digest-verified release manifests | Files older than 180 days are removed; retained entries are capped at 10 MB. |
