...
- [#560] Made future macOS patches operate on and verify the complete application bundle, including `Info.plist`; 1.8.7 intentionally uses a full-DMG transition so existing architecture-named installations cannot receive an incompatible in-place patch.
- [BUG-18112] Prevented passive startup, maintenance, metadata, and history destination probes from unlocking macOS Keychain or mounting SMB shares; credential access is deferred until an explicit destination test or actual backup needs it.
