...
public partial class MainWindow : Window
{
    ...
    private void SetSidebarCollapsed(bool collapsed)
    {
        ...
        if (RootGrid.ColumnDefinitions.Count >= 1)
            RootGrid.ColumnDefinitions[0].Width = new GridLength(Choose(collapsed, 86d, 260d));

        SidebarRoot.Padding = Choose(collapsed, new Thickness(10, 12, 10, 10), new Thickness(16, 18, 16, 14));
        SidebarTopSection.Margin = Choose(collapsed, new Thickness(0, 4, 0, 10), new Thickness(4, 10, 4, 16));
        NavButtonsPanel.Spacing = Choose(collapsed, 12d, 4d);
        NavButtonsPanel.Margin = Choose(collapsed, new Thickness(0, 24, 0, 10), new Thickness(0, 0, 0, 10));
        ...
        SidebarToggleGlyph.Data = StreamGeometry.Parse(Choose(collapsed, "M4.5 2 L9.5 7 L4.5 12", "M9.5 2 L4.5 7 L9.5 12"));
        SidebarToggleButton.HorizontalAlignment = Choose(collapsed, HorizontalAlignment.Center, HorizontalAlignment.Right);
        SidebarToggleButton.Margin = Choose(collapsed, new Thickness(0, 0, 0, 4), new Thickness(0, 0, 0, 2));
        string tooltipKey = Choose(collapsed, "Shell.SidebarExpand", "Shell.SidebarCollapse");
        string tooltipFallback = Choose(collapsed, "Expand sidebar", "Collapse sidebar");
        ToolTip.SetTip(
            SidebarToggleButton,
            LocalizationProvider.Service?.GetString(tooltipKey) ?? tooltipFallback);
        ...
        HorizontalAlignment contentAlignment = Choose(collapsed, HorizontalAlignment.Center, HorizontalAlignment.Left);
        HorizontalAlignment buttonAlignment = Choose(collapsed, HorizontalAlignment.Center, HorizontalAlignment.Stretch);
        double navButtonWidth = Choose(collapsed, 48d, double.NaN);
        double navButtonHeight = Choose(collapsed, 48d, double.NaN);
        Thickness navButtonPadding = Choose(collapsed, new Thickness(0), new Thickness(10, 8));
        ...
        NavDashboardButton.HorizontalAlignment = buttonAlignment;
        NavProjectsButton.HorizontalAlignment = buttonAlignment;
        NavBackupsButton.HorizontalAlignment = buttonAlignment;
        NavScheduleButton.HorizontalAlignment = buttonAlignment;
        NavHistoryButton.HorizontalAlignment = buttonAlignment;
        NavRecoveryButton.HorizontalAlignment = buttonAlignment;
        NavGuideButton.HorizontalAlignment = buttonAlignment;
        NavSettingsButton.HorizontalAlignment = buttonAlignment;
    ...
    private static T Choose<T>(bool collapsed, T collapsedValue, T expandedValue) =>
        collapsed ? collapsedValue : expandedValue;
