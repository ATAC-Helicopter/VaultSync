...
Desktop installers are published as assets on the repo's [Releases](https://github.com/ATAC-Helicopter/VaultSync/releases) page. Windows installers are produced with the `installer/VaultSyncInstaller.iss` Inno Setup script after publishing the `win-x64` output. macOS builds are shipped as unsigned `.dmg` images containing the `.app` bundle; users may need to right-click -> Open or clear quarantine (`xattr -dr com.apple.quarantine /Applications/VaultSync.app`). Linux builds ship as AppImage for x64, `.deb` packages for x64/arm64, and tarballs for x64/arm64; tarballs include a rootless `install.sh` that adds VaultSync to the user app menu and creates a `vaultsync` command under `~/.local`. macOS and Linux patches are delivered via platform-specific delta archives (see `docs/UPDATER.md`). The CLI follows the stable release line; run `dotnet tool update --global vaultsync.cli` after a release is published to stay in sync.
...
Prepared desktop beta release target: `1.7.4-Beta.3`. The published stable line remains `1.7.3` until the release is cut.
